...
init() {
    clear
    touch "$LOG_FILE"
    log "INFO" "=== ADIL RL-SWARM MANAGER STARTED ==="
}
...
show_header() {
    ...
    echo "│  ╚═╝░░╚═╝╚═════╝░╚═╝╚══════╝ │"
    echo "└─────────────────────────────┘"
    echo -e "${YELLOW}           🚀 Gensyn RL-Swarm Launcher by Adil 🚀${NC}"
    echo -e "${YELLOW}              GitHub: https://github.com/AdilWasnik${NC}"
    echo -e "${YELLOW}              Telegram: https://t.me/adil_wasnik${NC}"
    echo -e "${GREEN}===============================================================================${NC}"
}
...
run_fixall() {
    ...
    if curl -fsSL https://raw.githubusercontent.com/AdilWasnik/Gensyn-Ai-Node-Advanced-Installation-Guide/main/fix.sh | bash >/dev/null 2>&1; then
    ...
    while true; do
        ...
        case $choice in
            ...
            5)
                ...
                ;;
            6)
                echo -e "\n${GREEN}✅ Exiting... Thank you for using!${NC}"
                exit 0
                ;;
            *)
